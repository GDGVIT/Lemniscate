...
if(true)			
{
	...
	$sql_insert="INSERT INTO `db`.`post_table_$table_no` (`regno`, `post_text`, `no_of_likes_ans`, `status_qstn`, `date_time`, `pic_address`, `anoymous_status`, `total_no_replies`) VALUES ('$user_name', '$post', '0', '$status_post', '$d', '', '$anoymous_status', '0');";
	$res_insert=mysqli_query($mysqli,$sql_insert);
	if($res_insert)
	{
		//echo"Im here";
		echo 'Post added sucessfully!';
		$sql_display="SELECT * FROM `post_table_$table_no` WHERE regno='$user_name' AND date_time='$d'";
